...
| **Alist 存储** | ❌   | ❌   | ❌   | 不兼容   | 插件版本兼容风险，需单独复测 |

### Alist 不兼容说明

**问题原因**：Alist 插件与较新 Halo 附件状态模型存在兼容风险，当前主题不把它列为可用上传后端。
...
**状态**：应用市场当前最新为 `plugin-alist 1.1.3`，仍建议在较新 Halo 环境单独验证附件 `status` 初始化与预览链接；未验证通过前不要作为 Moments 前端发布的默认存储策略。
